...
Working directory должна быть project root, чтобы программа имела доступ к tmp/ 

Запускать тесты (после билда) соответсвенно:
```
cd Tape/
cmake-build-release/tests/tape_test
```

### Формат входных файлов

На вход приложению подаются 4 параметра: имя входного файла, имя выходного файла, N, M

Каждая ячейка ленты представлена в файле как 12 символов (знаки пробела и наше число, 
при этом число должно быть и должно быть ровно одно) и разделитель |. Пример в тестах.

Выходной файл либо существует и имеет столько же ячеек, сколько входной, либо он не существует и программа создает свой с правильной структурой. 
Иначе программа завершится с ошибкой (если ей дали меньше ячеек) или запишит поверх, оставив неиспользованные ячейки в конце файла.
